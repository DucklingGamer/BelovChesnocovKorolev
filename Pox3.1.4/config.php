...
// Функция создания таблиц заказов
function createOrderTables($pdo) {
    // (код такой же, как в user_config.php)
    $pdo->exec("
...
// ========== УПРАВЛЕНИЕ ТАЙМЕРАМИ ИГРЫ ДЛЯ АДМИНА ==========
...
// ========== ФУНКЦИЯ ДЛЯ ПОЛУЧЕНИЯ ОСТАВШЕГОСЯ ВРЕМЕНИ (ДЛЯ АДМИНА) ==========
function getRemainingGameTime() {
    if (!isset($_SESSION['game_state'])) {
        return ['type' => 'work', 'label' => 'До отдыха', 'remaining' => 0, 'minutes' => 0, 'seconds' => 0, 'formatted' => '00:00'];
    }
    ...
    if ($state == 'work') {
        $remaining = $_SESSION['work_time_seconds'] - $elapsed;
        $label = 'До отдыха';
    } else {
        $remaining = $_SESSION['break_time_seconds'] - $elapsed;
        $label = 'До работы';
    }
