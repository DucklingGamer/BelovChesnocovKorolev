...
// Получаем клиента из БД ЗАКАЗОВ (project_Belov) по email
...
// Если клиента нет, создаем его (БЕЗ user_id)
if (!$client_id && isset($_SESSION['email'])) {
    // Создаем клиента в БД заказов
    $stmt = $orders_pdo->prepare("
        INSERT INTO clients (full_name, email, phone, address) 
        VALUES (?, ?, ?, ?)
    ");
    $full_name = $_SESSION['username']; // или можно запросить у пользователя
    $address = ''; // нужно будет запросить при первом заказе
    
    $stmt->execute([$full_name, $_SESSION['email'], $_SESSION['phone'], $address]);
...
if (isset($_GET['cancel']) && $client_id) {
    ...
    $check = $orders_pdo->prepare("SELECT status FROM orders WHERE order_id = ? AND client_id = ?");
    ...
    if ($order && in_array($order['status'], ['pending', 'processing'])) {
        ...
        try {
            // Возвращаем товары на склад в БД КАТАЛОГА
            $items = $orders_pdo->prepare("SELECT product_id, quantity FROM order_items WHERE order_id = ?");
            ...
            // Обновляем статус заказа в БД ЗАКАЗОВ
            $orders_pdo->prepare("UPDATE orders SET status = 'cancelled' WHERE order_id = ?")->execute([$order_id]);
...
// Получаем заказы пользователя из БД ЗАКАЗОВ
if ($client_id) {
    $orders = $orders_pdo->prepare("
        SELECT o.*, 
