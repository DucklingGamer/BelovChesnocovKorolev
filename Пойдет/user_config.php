...
if (!in_array($current_page, $public_pages)) {
    ...
    try {
        // 2. Подключение к БД каталога (project_Chesnokov) - для товаров, категорий, услуг
        // ВАЖНО: Используем пользователя Chesnokov для доступа к project_Chesnokov
        ...
        // 3. Подключение к БД заказов (project_Belov) - для клиентов, заказов
        // Используем пользователя Belov для доступа к project_Belov
...
// Для обратной совместимости, если где-то используется старая переменная $pdo для заказов
// Но лучше везде использовать $orders_pdo для заказов
?>